...
export default function WishlistPage() {
  ...
  return <div className="animate-rise"><h1 className="text-2xl font-bold tracking-tight">Wishlist</h1><div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{products.map((product) => <Link href={`/product/${product.id}`} key={product.id} className="card overflow-hidden"><div className="aspect-[3/4] bg-neutral-100">{product.image_url && <img src={product.image_url} alt={product.name} className="h-full w-full object-cover" />}</div><div className="p-4"><p className="font-medium">{product.name}</p><p className="mt-1 text-sm text-neutral-600">{formatCurrency(product.price)}</p></div></Link>)}</div></div>;
}
